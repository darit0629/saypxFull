...
// Range-aware variant for HTTP streaming (video seeking needs real byte
// ranges, not just the whole file every time) - returns everything a proxy
// route needs to mirror the response correctly, partial-content included.
async function downloadStream(key, options = {}) {
  ...
  return {
    ...
    isPartial: Boolean(res.ContentRange),
  };
}
